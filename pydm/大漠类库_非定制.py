...
if getattr(sys, 'frozen', False):
    # 打包后:使用exe所在目录
    _local_dir = os.path.dirname(sys.executable)
else:
    # 开发环境:src32的父目录
...
class DM:
    def __init__(self, 注册账号="", 注册密码="", dm=None):
        if dm is None:
            self.dm = None
            reg_path = os.path.join(_local_dir, "DmReg.dll")
            dm_path = os.path.join(_local_dir, "dm.dll")
            if not os.path.exists(reg_path):
                raise ValueError("免注册DmReg.dll 路径不存在 %s" % reg_path)
            if not os.path.exists(dm_path):
                raise ValueError("dm.dll 路径不存在 %s" % dm_path)
            # print('正在免注册调用')  # 注释掉避免干扰subprocess
            ...
            # print('免注册调用成功 版本号为:', self.Ver())  # 注释掉避免干扰subprocess
            self.reg(注册账号, 注册密码)
        else:
            self.dm = dm

    # def __del__(self):
    #     GetBindWindow = getattr(self.dm, "GetBindWindow", None)
    #     if GetBindWindow is not None:
    #         self.UnBindWindow()
    ...
    def reg(self, dm_user, dm_pass):
        ...
        # print("大漠注册结果:", dm_res[res])  # 注释掉避免干扰subprocess
        return self.dm

    @staticmethod
    def CreateObject():
        """创建线程大漠对象（需要先创建全局大漠）"""
        if not DM._dmreg_initialized:
            raise RuntimeError("必须先创建全局大漠对象（DM类实例）才能创建线程大漠")
...
if __name__ == "__main__":
    ...
    # 3.有了全局大漠,就可以创建线程大漠了
    dm2 = dm.CreateObject()  # 创建线程大漠
    hwnd = 461310  # 记事本句柄
    res = dm2.BindWindowEx(hwnd, "gdi", "windows", "windows", "", 0)
    if not res:
        raise ValueError("绑定失败")
    dm2.SendString2(hwnd, "hello world") # 第一次发送测试
    td_info[0].dm = dm2  # 回写数据到全局引用 0就是第一个线程大漠
    # 如果在别的地方引用线程大漠,需要这样写
    dm2 = td_info[0].dm
    dm2.SendString2(hwnd, "22222") # 第二次发送测试
    print("ok")
